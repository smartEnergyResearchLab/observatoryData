...
All of the import read types have some participants for which there were no valid readings available, however over 900 participants have at least 95% of gas and electricity active import reads available. A handful of participants are recorded as “with a device” for half-hourly data but not for daily. This is because these participants withdrew consent after we started collecting half-hourly data, but before daily reading collection began.
...
Table 2 shows the same data presented as a percentage. The first row of data (‘With a device’) shows the percent of participants we expect to have each type of read. Approximately 75% of households have a gas meter registered with the DCC (i.e. a SMETS2 gas meter or an upgraded SMETS1 meter). The percentages in the rows that follow are percentage of those with a device, e.g. 79.6% of households with a smart meter that we could collect data from have at least 90% of their half-hourly electricity active import readings available and valid.
