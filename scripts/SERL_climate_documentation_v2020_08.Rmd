...
climate_folder <- "S:/ENERGINST_EaB_Project_17_SMRP/Data/Researcher data/Climate data/"
...
climate_filename <- get.serl.filename("climate_data", release_version)
...
save(climate,
     file = paste(climate_folder, climate_filename, ".RData", sep = "")
     )

} else {
  load(paste(climate_folder, climate_filename, ".RData", sep = ""))
}

if(write_climate == TRUE) {
  fwrite(climate,
         file = paste(saving_location, "Final/", climate_filename, ".csv", sep = ""))
}
...
More variables will be provided in future releases, but at the moment only temperature at 2 metres is part of the data. It can be linked with SERL participants using the 'gridCell' variable. The filename of this dataset is *`r paste(climate_filename, ".csv", sep = "")`*. This documentation will be expanded in future as more climate variables are provided with SERL datasets. 
...
climate_fields[columnNames == "gridCell", `:=`(description = "Grid cell for linking to participant data",
                                           units = "NA",
                                           example = "38_31")]

climate_fields[columnNames == "analysisDate", `:=`(description = "Date the data refers to",
                                           units = "NA",
                                           example = "2018-08-01")]

climate_fields[columnNames == "utcDateTime", `:=`(description = "Time and date of the data in UTC",
                                           units = "%Y-%m-%d %H:%M:%S",
                                           example = "2018-08-01 00:00:00")]


climate_fields[columnNames == "temperature_2_metres_K", `:=`(description = "Temperature at 2 metres from the surface",
                                           units = "deg K",
                                           example = "285.12")]


climate_fields[columnNames == "temperature_2_metres_C", `:=`(description = "Temperature at 2 metres from the surface converted into Celsius",
                                           units = "deg C",
                                           example = "12.12")]
...
stats[Statistic == "Earliest read date", Value := as.character(climate[1, analysisDate])]
stats[Statistic == "Latest read date", Value := as.character(climate[nrow(climate), analysisDate])]
stats[Statistic == "Lowest temperature (deg C)", Value := climate[, min(temperature_2_metres_C)]]
stats[Statistic == "Highest temperature (deg C)", Value := climate[, max(temperature_2_metres_C)]]
stats[Statistic == "Mean temperature (deg C)", Value := climate[, round(mean(temperature_2_metres_C), 2)]]
stats[Statistic == "Number of grid cells", Value := length(unique(climate$gridCell))]
