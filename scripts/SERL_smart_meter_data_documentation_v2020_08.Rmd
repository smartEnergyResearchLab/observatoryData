---
title: "Smart meter consumption data: Technical documentation"
output: 
  word_document:
...
sm_processing_filename <- "SERL_smart_meter_data_processing_v2020_08.R"
...
if(load_table_data == FALSE) {
  ...
  load(paste(saving_location, "sm_data_documentation_input.RData", sep = ""))
}
```
...
add.simple.table.superscript <- function(flex_table,
  ...
  formatted_table <- compose(flex_table, 
                         i = row_ids, 
                         j = col_name,
                         part = t_part,
                         value = as_paragraph(
...
read.type.tab <- read.type.tab[, .(deviceType, readType, units, description)]
setnames(read.type.tab, 
         old = c("units", "description"),
         new = c("Units", "Description")
         )

read.type.tab <- my.flex(read.type.tab)

read.type.tab <- add.simple.table.superscript(read.type.tab,
                                              row_ids = c(2, 5))
...
hh.cols.tab <- my.flex(hh.cols.tab)
hh.cols.tab <- add.simple.table.superscript(hh.cols.tab,
                                            row_ids = 17)
...
if(load_table_data == FALSE) {
  ...
  d.cols.tab[columnNames == "Read_date_time", `:=`(description = "Time and date of read",
                                                    units = "%Y-%m-%d",
                                                    example = "2019-11-01")]
...
d.cols.tab <- my.flex(d.cols.tab)
d.cols.tab <- add.simple.table.superscript(d.cols.tab,
                                           row_ids = c(16, 20, 21))
...
sched.tab <- my.flex(sched.tab)
sched.tab <- compose(sched.tab,
                     i = 21:25,
                     j = "Units",
                     part = "body",
                     value = as_paragraph(
