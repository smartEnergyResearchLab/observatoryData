...
The survey dataset contains `r nrow(survey_clean)` rows (one per participant who responded) and `r ncol(survey_clean)` columns. 
...
presentable_questions <- questionTable[!Variable %in% c("Respondent_ID", "Respondent_Serial"),
...
presentable2 <- questionTable[!Variable %in% c("Respondent_ID", "Respondent_Serial"),
                                       list(Variable, i.Label,freeText, Value, Label)]
...
otherCols.tab[columnNames == "A5_edit", `:=`(description = "TRUE if A5 response was very high and therefore assumed to be in Fahrenheit and converted to Celcius, otherwise FASLE.",
                                         example = "FALSE")]
...
otherCols.tab[columnNames == "A16_edit", 
              `:=`(description = "",
                                         example = "TRUE if any A16 responses were edited, otherwise FALSE. If 'not applicable' or 'Not considering any changes' were selected, any other response was removed (except for 'other' which was kept to prevent deleting potentially useful notes), ")]
